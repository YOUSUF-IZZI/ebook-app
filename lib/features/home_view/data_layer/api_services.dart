...
class ApiServices
{
  ...
  Future<Either<Failure, List<BooksModel>>> getNewestBooks() async
  {
    try
    {
      final response = await _dio.get('${baseUrl}volumes?Filtering=free-ebooks&Sorting=newest&q=subject:Programming');
  ...
  Future<Either<Failure, List<BooksModel>>> getFeaturedBooks() async
  {
    try
    {
      final response = await _dio.get('${baseUrl}volumes?Filtering=free-ebooks&q=subject:Programming');
      if(response.statusCode == 200)
      {
        final List<BooksModel> books = [];
        for(var item in response.data['items'])
